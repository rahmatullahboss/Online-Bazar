...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    const config = {
      SIM_RESOLUTION: isMobile ? 64 : 16, // Reduced from 128 to 64 for mobile, 16 for desktop
      DYE_RESOLUTION: isMobile ? 720 : 256, // Reduced from 1440 to 720 for mobile, 256 for desktop
      CAPTURE_RESOLUTION,
      DENSITY_DISSIPATION,
      VELOCITY_DISSIPATION,
      PRESSURE,
      PRESSURE_ITERATIONS: isMobile ? 15 : 10, // Reduced from 20 to 15 for mobile, 10 for desktop
      CURL: isMobile ? 2 : 1, // Reduced from 3 to 2 for mobile, 1 for desktop
      SPLAT_RADIUS: isMobile ? 0.1 : 0.025, // Reduced from 0.2 to 0.1 for mobile, 0.025 for desktop
      SPLAT_FORCE: isMobile ? 3000 : 1000, // Reduced from 6000 to 3000 for mobile, 1000 for desktop
      SHADING: isMobile ? false : false, // Disable shading for both but ensure it's consistent
      COLOR_UPDATE_SPEED,
      PAUSED: false,
      BACK_COLOR,
      TRANSPARENT,
    ...
    class Material {
      ...
      setKeywords(keywords: string[]) {
        ...
        if (program == null) {
          // Add null check for gl before using it
    ...
    function initFramebuffers() {
      // Add null checks for gl and ext
      if (!gl || !ext) {
        console.warn('WebGL context or extensions not available')
        return
      }
    ...
    function updateColors(dt: number) {
      ...
      if (colorUpdateTimer >= 1) {
        ...
        pointers.forEach((p) => {
          ...
          // Fix type mismatch - convert object to array
    ...
    function clickSplat(pointer: Pointer) {
      ...
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, [color.r, color.g, color.b])
    }

    function splat(x: number, y: number, dx: number, dy: number, color: [number, number, number]) {
      // Add null check for gl and canvas
      if (!gl || !canvas) {
        console.warn('WebGL context or canvas is not available')
        return
      }
    ...
    function generateColor(): { r: number; g: number; b: number } {
      ...
      c.r *= 0.05 // Decreased from 0.15 to 0.05 for lower opacity on desktop
      c.g *= 0.05 // Decreased from 0.15 to 0.05 for lower opacity on desktop
      c.b *= 0.05 // Decreased from 0.15 to 0.05 for lower opacity on desktop
      return c
    }
